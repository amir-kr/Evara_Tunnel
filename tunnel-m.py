...
async def ping_ipv6_ssh(host, username, password, target_ip, message, operation="بررسی وضعیت"):
    await bot.send_message(
        ...
        text=escape_md(f"⏳ لطفاً منتظر بمانید، در حال {operation} هستیم..."),
...
async def start_command(message: types.Message, state: FSMContext):
    ...
    welcome_msg = escape_md(
        "🌟 خوش اومدی به ربات مدیریت تونل اوارا 🌟\n\n"
        f"شما به‌عنوان {'مدیر' if role == 'admin' else 'کاربر'} وارد شده‌اید.\n"
        f" https://t.me/evara_tu :برای اطلاع از به‌روزرسانی‌های ربات و حمایت از ما وارد کانال ما شوید."
    )
    try:
        ...
        await bot.send_message(
            chat_id=message.chat.id,
            text=escape_md("❌ خطایی رخ داد. لطفاً دوباره تلاش کنید."),
            reply_markup=get_main_menu_keyboard(),
            parse_mode="MarkdownV2"
        )
...
async def select_tunnel(message: types.Message, state: FSMContext):
    ...
    if not iran_ipv6 or not kharej_ipv6:
        await bot.send_message(
            chat_id=message.chat.id,
            text=escape_md("⚠️ آدرس‌های IPv6 برای این تونل یافت نشدند. لطفاً تونل را دوباره ایجاد کنید."),
            reply_markup=get_main_menu_keyboard(),
            parse_mode="MarkdownV2"
        )
        await ServerConfig.MainMenu.set()
    ...
    if not is_valid_ipv6(iran_ipv6) or not is_valid_ipv6(kharej_ipv6):
        await bot.send_message(
            chat_id=message.chat.id,
            text=escape_md("⚠️ آدرس‌های IPv6 نامعتبر هستند. لطفاً تونل را دوباره ایجاد کنید."),
            reply_markup=get_main_menu_keyboard(),
            parse_mode="MarkdownV2"
        )
        await ServerConfig.MainMenu.set()
    ...
    response += f"🌍 *سرور ایران ({escape_md(iran_ipv6)}):*\n"
    ...
    response += f"🌎 *سرور خارج ({escape_md(kharej_ipv6)}):*\n"
...
async def process_crontab_hour(message: types.Message, state: FSMContext):
    ...
    if "خطا" in result:
        await bot.send_message(
            chat_id=message.chat.id,
            text=escape_md(f"❌ خطا در تنظیم کرون‌تب سرور ایران: {result}"),
            parse_mode="MarkdownV2"
        )
        await back_to_main_menu(message, state)
        return
    ...
    if "خطا" in result:
        await bot.send_message(
            chat_id=message.chat.id,
            text=escape_md("❌ خطا در تنظیم کرون‌تبل سرور خارج: {result}"),
            parse_mode="MarkdownV2"
        )
        await back_to_main_menu(message, state)
        return
...
async def execute_ssh_command(host, username, password, command):
    ...
    try:
        ...
        logger.error(f"SSH command failed: {command}, error: {str(e)}")
        return f"خطا: {str(e)}"
    finally:
        if ssh:
            ssh.close()
