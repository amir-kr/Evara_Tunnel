...
async def ping_ipv6_ssh(host, username, password, target_ip, message, operation="بررسی وضعیت"):
    await bot.send_message(
        chat_id=message.chat.id,
        text=f"⏳ *لطفاً منتظر بمانید، در حال {operation} هستیم...*",
        parse_mode="Markdown"
    )
    try:
        command = f"ping6 -c 4 {target_ip}"
        ...
        if packet_loss and int(packet_loss.group(1)) == 0:
            ...
            return {"status": "disconnected", "rtt": "N/A"}
...
async def start_command(message: types.Message, state: FSMContext):
    ...
    if not role:
        await bot.send_message(
            chat_id=message.chat.id,
            text="❌ *متأسفیم، شما دسترسی استفاده از این ربات را ندارید. لطفاً با مدیر تماس بگیرید.*",
            parse_mode="Markdown"
        )
        return
    
    welcome_msg = (
        "🌟 *خوش اومدی به ربات مدیریت تانل اوارا* 🌟\n\n"
        f"شما به‌عنوان *{'مدیر' if role == 'admin' else 'کاربر'}* وارد شده‌اید.\n"
        ":https://t.me/evara_tuبرای اطلاع از بروزرسانی های ربات و حمایت از ما وارد کانال ما بشین "
    )
    await bot.send_message(
        chat_id=message.chat.id,
        text=welcome_msg,
...
async def main_menu(message: types.Message, state: FSMContext):
    ...
    if not role:
        await bot.send_message(
            chat_id=message.chat.id,
            text="❌ *دسترسی غیرمجاز! لطفاً دوباره وارد شوید.*",
            parse_mode="Markdown"
        )
        await state.finish()
        return
    ...
    if message.text == "🚀 ساخت تونل جدید":
        ...
        await bot.send_message(
            chat_id=message.chat.id,
            text="✨ *لطفاً یک نام برای تونل جدید وارد کنید:*",
        ...
        if not tunnels:
            ...
            await bot.send_message(
                chat_id=message.chat.id,
                text="🔍 *لطفاً تونل موردنظر را برای بررسی انتخاب کنید:*",
        ...
        if not tunnels:
            await bot.send_message(
                chat_id=message.chat.id,
                text="⚠️ *هیچ تونلی برای حذف یافت نشد!*",
                reply_markup=get_main_menu_keyboard(),
                parse_mode="Markdown"
            )
            await ServerConfig.MainMenu.set()
            ...
            await bot.send_message(
                chat_id=message.chat.id,
                text="🗑 *لطفاً تونل موردنظر را برای حذف انتخاب کنید:*",
                reply_markup=keyboard,
                parse_mode="Markdown"
            )
        ...
        await bot.send_message(
            chat_id=message.chat.id,
            text="❌ *لطفاً گزینه‌ای معتبر انتخاب کنید!*",
            reply_markup=get_main_menu_keyboard(),
            parse_mode="Markdown"
        )
...
async def select_tunnel(message: types.Message, state: FSMContext):
    ...
    if not tunnel:
        await bot.send_message(
            chat_id=message.chat.id,
            text="⚠️ *تونل با این نام یافت نشد یا متعلق به شما نیست!*",
            reply_markup=get_main_menu_keyboard(),
            parse_mode="Markdown"
        )
        await ServerConfig.MainMenu.set()
    ...
    if not iran_ipv6 or not kharej_ipv6:
        await bot.send_message(
            chat_id=message.chat.id,
            text="⚠️ *آدرس‌های IPv6 برای این تونل یافت نشدند. لطفاً تونل را دوباره ایجاد کنید.*",
            reply_markup=get_main_menu_keyboard(),
            parse_mode="Markdown"
        )
        await ServerConfig.MainMenu.set()
    ...
    if not is_valid_ipv6(iran_ipv6) or not is_valid_ipv6(kharej_ipv6):
        await bot.send_message(
            chat_id=message.chat.id,
            text="⚠️ *آدرس‌های IPv6 نامعتبر هستند. لطفاً تونل را دوباره ایجاد کنید.*",
            reply_markup=get_main_menu_keyboard(),
            parse_mode="Markdown"
        )
        await ServerConfig.MainMenu.set()
    ...
    iran_ping = await ping_ipv6_ssh(iran_server_ip, iran_username, iran_password, kharej_ipv6, message, "بررسی وضعیت تونل ایران")
    kharej_ping = await ping_ipv6_ssh(kharej_server_ip, kharej_username, kharej_password, iran_ipv6, message, "بررسی وضعیت تونل خارج")
    
    
    response = f"📊 *وضعیت تونل '{tunnel_name}'* 📊\n\n"
    if role == 'admin':
        response += f"👤 *کاربر:* {tunnel_user_id}\n"
    response += f"🌍 *سرور ایران ({iran_ipv6}):*\n"
...
async def delete_tunnel(message: types.Message, state: FSMContext):
    ...
    await bot.send_message(
        chat_id=message.chat.id,
        text=f"⏳ *لطفاً منتظر بمانید، در حال حذف تونل '{tunnel_name}' هستیم...*",
        parse_mode="Markdown"
    )
    ...
    iran_cleanup_commands = [
        ...
        "sudo ip tun del GRE6Tun_To_IR",
        "sudo ip tun del 6to4_To_IR",
        "crontab -r"
    ]
    ...
    kharej_cleanup_commands = [
        ...
        "sudo rm -f /etc/ipsec.secrets",
        "sudo rm -f /usr/local/bin/recycle-gre-ipsec.sh",
        "sudo ip tun del GRE6Tun_To_KH",
        "sudo ip tun del 6to4_To_KH",
        "crontab -r"
    ]
    ...
    for cmd in iran_cleanup_commands:
        ...
        if "خطا" in result:
            await bot.send_message(
                chat_id=message.chat.id,
                text=f"❌ *خطا در حذف تنظیمات سرور ایران:* {result}",
                reply_markup=get_main_menu_keyboard(),
                parse_mode="Markdown"
            )
            await ServerConfig.MainMenu.set()
    ...
    for cmd in kharej_cleanup_commands:
        ...
        if "خطا" in result:
            await bot.send_message(
                chat_id=message.chat.id,
                text=f"❌ *خطا در حذف تنظیمات سرور خارج:* {result}",
                reply_markup=get_main_menu_keyboard(),
                parse_mode="Markdown"
            )
            await ServerConfig.MainMenu.set()
    ...
    await bot.send_message(
        chat_id=message.chat.id,
        text=f"✅ *تونل '{tunnel_name}' با موفقیت از هر دو سرور و دیتابیس حذف شد!*",
        reply_markup=get_main_menu_keyboard(),
        parse_mode="Markdown"
    )
    await ServerConfig.MainMenu.set()
...
async def process_tunnel_name(message: types.Message, state: FSMContext):
    ...
    if not tunnel_name:
        await bot.send_message(
            chat_id=message.chat.id,
            text="❌ *لطفاً یک نام معتبر برای تونل وارد کنید:*",
            reply_markup=get_back_buttons(),
            parse_mode="Markdown"
        )
        return
    ...
    await bot.send_message(
        chat_id=message.chat.id,
        text="🔗 *لطفاً نوع تونل را انتخاب کنید:*",
...
async def tunnel_menu(message: types.Message, state: FSMContext):
    if message.text == "🔗 تونل 1 ایران به 1 خارج":
        await bot.send_message(
            chat_id=message.chat.id,
            text="🌍 *لطفاً IP سرور ایران را برای اتصال SSH وارد کنید:*",
            reply_markup=get_back_buttons(),
            parse_mode="Markdown"
        )
...
async def process_iran_server_ip(message: types.Message, state: FSMContext):
    ...
    if message.text == "⬅️ بازگشت به مرحله قبل":
        await bot.send_message(
            chat_id=message.chat.id,
            text="✨ *لطفاً یک نام برای تونل وارد کنید:*",
            reply_markup=get_back_buttons(),
            parse_mode="Markdown"
        )
        await ServerConfig.TunnelName.set()
    ...
    if not is_valid_ip(message.text.strip()):
        await bot.send_message(
            chat_id=message.chat.id,
            text="❌ *لطفاً یک IP معتبر وارد کنید (مثلاً 192.168.1.1):*",
            reply_markup=get_back_buttons(),
            parse_mode="Markdown"
        )
        return
    ...
    await bot.send_message(
        chat_id=message.chat.id,
        text="👤 *لطفاً نام کاربری سرور ایران را وارد کنید:*",
        reply_markup=get_back_buttons(),
        parse_mode="Markdown"
    )
...
async def process_iran_username(message: types.Message, state: FSMContext):
    ...
    await bot.send_message(
        chat_id=message.chat.id,
        text="🔒 *لطفاً رمز عبور سرور ایران را وارد کنید:*",
        reply_markup=get_back_buttons(),
        parse_mode="Markdown"
    )
...
async def process_iran_password(message: types.Message, state: FSMContext):
    ...
    await bot.send_message(
        chat_id=message.chat.id,
        text="⏳ *لطفاً منتظر بمانید، در حال تست اتصال به سرور ایران هستیم...*",
        parse_mode="Markdown"
    )
    ...
    try:
        ...
        await bot.send_message(
            chat_id=message.chat.id,
            text="✅ *با موفقیت به سرور ایران متصل شد!*",
            parse_mode="Markdown"
        )
    except Exception as e:
        await bot.send_message(
            chat_id=message.chat.id,
            text=f"❌ *اتصال به سرور ایران ناموفق بود:* {str(e)}\n*لطفاً دوباره اطلاعات را وارد کنید.*",
            parse_mode="Markdown"
        )
        await back_to_main_menu(message, state)
        return
    ...
    await bot.send_message(
        chat_id=message.chat.id,
        text="🌎 *لطفاً IP سرور خارج را برای اتصال SSH وارد کنید:*",
        reply_markup=get_back_buttons(),
        parse_mode="Markdown"
    )
...
async def process_kharej_server_ip(message: types.Message, state: FSMContext):
    ...
    if not is_valid_ip(message.text.strip()):
        await bot.send_message(
            chat_id=message.chat.id,
            text="❌ *لطفاً یک IP معتبر وارد کنید (مثلاً 192.168.1.2):*",
            reply_markup=get_back_buttons(),
            parse_mode="Markdown"
        )
        return
    ...
    await bot.send_message(
        chat_id=message.chat.id,
        text="👤 *لطفاً نام کاربری سرور خارج را وارد کنید:*",
        reply_markup=get_back_buttons(),
        parse_mode="Markdown"
    )
...
async def process_kharej_username(message: types.Message, state: FSMContext):
    ...
    await bot.send_message(
        chat_id=message.chat.id,
        text="🔒 *لطفاً رمز عبور سرور خارج را وارد کنید:*",
        reply_markup=get_back_buttons(),
        parse_mode="Markdown"
    )
...
async def process_kharej_password(message: types.Message, state: FSMContext):
    ...
    await bot.send_message(
        chat_id=message.chat.id,
        text="⏳ *لطفاً منتظر بمانید، در حال تست اتصال به سرور خارج هستیم...*",
        parse_mode="Markdown"
    )
    ...
    try:
        ...
        await bot.send_message(
            chat_id=message.chat.id,
            text="✅ *با موفقیت به سرور خارج متصل شد!*",
            parse_mode="Markdown"
        )
    except Exception as e:
        await bot.send_message(
            chat_id=message.chat.id,
            text=f"❌ *اتصال به سرور خارج ناموفق بود:* {str(e)}\n*لطفاً دوباره اطلاعات را وارد کنید.*",
            parse_mode="Markdown"
        )
        await back_to_main_menu(message, state)
        return
    ...
    await bot.send_message(
        chat_id=message.chat.id,
        text="⏳ *لطفاً منتظر بمانید، در حال نصب پیش‌نیازها روی سرورها هستیم...*",
        parse_mode="Markdown"
    )
    ...
    for cmd in initial_commands:
        ...
        if "خطا" in result:
            await bot.send_message(
                chat_id=message.chat.id,
                text=f"❌ *خطا در نصب پیش‌نیازها روی سرور ایران:* {result}",
                parse_mode="Markdown"
            )
            await back_to_main_menu(message, state)
            return
    ...
    for cmd in initial_commands:
        ...
        if "خطا" in result:
            await bot.send_message(
                chat_id=message.chat.id,
                text=f"❌ *خطا در نصب پیش‌نیازها روی سرور خارج:* {result}",
                parse_mode="Markdown"
            )
            await back_to_main_menu(message, state)
            return
    ...
    await bot.send_message(
        chat_id=message.chat.id,
        text="✅ *پیش‌نیازها با موفقیت روی هر دو سرور نصب شدند!*",
        parse_mode="Markdown"
    )
    await bot.send_message(
        chat_id=message.chat.id,
        text="🌍 *لطفاً IP سرور ایران را وارد کنید:*",
        reply_markup=get_back_buttons(),
        parse_mode="Markdown"
    )
...
async def process_iran_ip(message: types.Message, state: FSMContext):
    ...
    if not is_valid_ip(message.text.strip()):
        await bot.send_message(
            chat_id=message.chat.id,
            text="❌ *لطفاً یک IP معتبر برای سرور ایران وارد کنید (مثلاً 192.168.1.1):*",
            reply_markup=get_back_buttons(),
            parse_mode="Markdown"
        )
        return
    ...
    await bot.send_message(
        chat_id=message.chat.id,
        text="🌎 *لطفاً IP سرور خارج را وارد کنید:*",
        reply_markup=get_back_buttons(),
        parse_mode="Markdown"
    )
...
async def process_kharej_ip(message: types.Message, state: FSMContext):
    ...
    if not is_valid_ip(message.text.strip()):
        await bot.send_message(
            chat_id=message.chat.id,
            text="❌ *لطفاً یک IP معتبر برای سرور خارج وارد کنید (مثلاً 192.168.1.2):*",
            reply_markup=get_back_buttons(),
            parse_mode="Markdown"
        )
        return
    ...
    await bot.send_message(
        chat_id=message.chat.id,
        text="🔑 *لطفا یک رمز سخت برای تانل وارد کنید *",
        reply_markup=get_back_buttons(),
        parse_mode="Markdown"
    )
...
async def process_psk(message: types.Message, state: FSMContext):
    ...
    await bot.send_message(
        chat_id=message.chat.id,
        text="📏 *لطفاً MTU برای تونل 6to4 را انتخاب کنید:*",
...
async def process_mtu_6to4_selection(callback_query: types.CallbackQuery, state: FSMContext):
    ...
    if callback_query.data == "mtu_6to4_default":
        ...
        try:
            await callback_query.message.edit_text(
                text="✅ *MTU برای تونل 6to4 به‌صورت پیش‌فرض (1480) تنظیم شد.*\n📏 *لطفاً MTU برای تونل GRE را انتخاب کنید:*",
        ...
        try:
            await callback_query.message.edit_text(
                text="✍️ *لطفاً مقدار MTU برای تونل 6to4 را به‌صورت دستی وارد کنید (بین 1280 و 1500):*",
                reply_markup=get_back_buttons(),
                parse_mode="Markdown"
            )
        except:
...
async def process_manual_mtu_6to4(message: types.Message, state: FSMContext):
    ...
    try:
        ...
        if 1280 <= mtu <= 1500:
            ...
            await bot.send_message(
                chat_id=message.chat.id,
                text=f"✅ *MTU برای تونل 6to4 روی {mtu} تنظیم شد.*",
                parse_mode="Markdown"
            )
            await bot.send_message(
                chat_id=message.chat.id,
                text="📏 *لطفاً MTU برای تونل GRE را انتخاب کنید:*",
                reply_markup=get_mtu_gre_selection_keyboard(),
                parse_mode="Markdown"
            )
            await ServerConfig.MTU_GRE.set()
            ...
            await bot.send_message(
                chat_id=message.chat.id,
                text="❌ *لطفاً مقدار MTU بین 1280 و 1500 وارد کنید:*",
                reply_markup=get_back_buttons(),
                parse_mode="Markdown"
            )
    except ValueError:
        await bot.send_message(
            chat_id=message.chat.id,
            text="❌ *لطفاً یک عدد معتبر برای MTU وارد کنید:*",
            reply_markup=get_back_buttons(),
            parse_mode="Markdown"
        )
...
async def process_mtu_gre_selection(callback_query: types.CallbackQuery, state: FSMContext):
    ...
    if callback_query.data == "mtu_gre_default":
        ...
        try:
            await callback_query.message.edit_text(
                text="✅ *MTU برای تونل GRE به‌صورت پیش‌فرض (1424) تنظیم شد.*",
                parse_mode="Markdown"
            )
        except:
            await bot.send_message(
        ...
        try:
            await callback_query.message.edit_text(
                text="✍️ *لطفاً مقدار MTU برای تونل GRE را به‌صورت دستی وارد کنید (بین 1280 و 1500):*",
                reply_markup=get_back_buttons(),
                parse_mode="Markdown"
            )
        except:
...
async def process_manual_mtu_gre(message: types.Message, state: FSMContext):
    ...
    try:
        ...
        if 1280 <= mtu <= 1500:
            ...
            await bot.send_message(
                chat_id=message.chat.id,
                text=f"✅ *MTU برای تونل GRE روی {mtu} تنظیم شد.*",
                parse_mode="Markdown"
            )
...
async def process_config_files(message: types.Message, state: FSMContext):
    ...
    await bot.send_message(
        chat_id=message.chat.id,
        text="⏳ *لطفاً منتظر بمانید، در حال نصب تونل روی سرورها هستیم...*",
        parse_mode="Markdown"
    )
...
"""
    ...
    for cmd in iran_commands:
        ...
        if "خطا" in result:
            await bot.send_message(
                chat_id=message.chat.id,
                text=f"❌ *خطا در پیکربندی سرور ایران:* {result}",
                parse_mode="Markdown"
            )
            await back_to_main_menu(message, state)
            return
    ...
    for cmd in kharej_commands:
        ...
        if "خطا" in result:
            await bot.send_message(
                chat_id=message.chat.id,
                text=f"❌ *خطا در پیکربندی سرور خارج:* {result}",
                parse_mode="Markdown"
            )
            await back_to_main_menu(message, state)
            return
    ...
    await bot.send_message(
        chat_id=message.chat.id,
        text="✅ *تونل با موفقیت نصب شد!*",
        parse_mode="Markdown"
    )
    await bot.send_message(
        chat_id=message.chat.id,
        text=f"🔗 *تونل را برای سرور ایران با آی‌پی زیر پینگ کنید:* {kharej_ipv6}",
        parse_mode="Markdown"
    )
    await bot.send_message(
        chat_id=message.chat.id,
        text="⏰ *لطفاً ساعت را برای ریست تونل وارد کنید (0-23):*",
        reply_markup=get_back_buttons(),
        parse_mode="Markdown"
    )
...
async def process_crontab_hour(message: types.Message, state: FSMContext):
    ...
    if not is_valid_crontab_hour(crontab_hour):
        await bot.send_message(
            chat_id=message.chat.id,
            text="❌ *لطفاً یک ساعت معتبر بین 0 تا 23 وارد کنید:*",
            reply_markup=get_back_buttons(),
            parse_mode="Markdown"
        )
        return
    ...
    await bot.send_message(
        chat_id=message.chat.id,
        text="⏳ *لطفاً منتظر بمانید، در حال تنظیم زمان‌بندی ریست تونل هستیم...*",
        parse_mode="Markdown"
    )
    ...
    if "خطا" in result:
        await bot.send_message(
            chat_id=message.chat.id,
            text=f"❌ *خطا در تنظیم کرون‌تب سرور ایران:* {result}",
            parse_mode="Markdown"
        )
        await back_to_main_menu(message, state)
        return
    ...
    if "خطا" in result:
        await bot.send_message(
            chat_id=message.chat.id,
            text=f"❌ *خطا در تنظیم کرون‌تب سرور خارج:* {result}",
            parse_mode="Markdown"
        )
        await back_to_main_menu(message, state)
        return
    ...
    await bot.send_message(
        chat_id=message.chat.id,
        text="✅ *تنظیم ریست تونل برای هر دو سرور با موفقیت انجام شد!*",
        parse_mode="Markdown"
    )
    await bot.send_message(
        chat_id=message.chat.id,
        text="🎉 *نصب تونل با موفقیت به پایان رسید!*",
...
async def execute_ssh_command(host, username, password, command):
    try:
        ssh = paramiko.SSHClient()
        ssh.set_missing_host_key_policy(paramiko.AutoAddPolicy())
